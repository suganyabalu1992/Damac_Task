package com.damac.srvc.impl;

import static com.damac.util.Constants.DATE_ERROR_MSG;
import static com.damac.util.Constants.EMP_EXIST;
import static com.damac.util.Constants.EMP_NOT_FOUND;
import static com.damac.util.Constants.INVALID_DEPARTMENT;
import static com.damac.util.Constants.SUCCESS;
...
import com.damac.entity.EmployeeEntity;
import com.damac.exception.BusinessException;
...
import com.damac.repo.DepartmentRepo;
import com.damac.repo.EmployeeRepo;
...
public class EmployeeAppSrvcImpl implements EmployeeAppSrvc {
	...
	public List<EmployeeRespVO> getEmployee(SearchEmpReq searchEmpReq) {
		...
		List<EmployeeEntity> employeeEntities = new LinkedList<EmployeeEntity>();
		if(null != searchEmpReq.getEmployeeName() && null != searchEmpReq.getEmployeeNo())
		    employeeEntities = employeeRepo.findByEmployeeNoAndEmployeeNameContainingIgnoreCase(
				searchEmpReq.getEmployeeNo(), searchEmpReq.getEmployeeName());
		else if(null != searchEmpReq.getEmployeeNo())
			emp = employeeRepo.findByEmployeeNo(searchEmpReq.getEmployeeNo());
		else if(null != searchEmpReq.getEmployeeName())
			employeeEntities = employeeRepo.findByEmployeeNameContainingIgnoreCase(searchEmpReq.getEmployeeName());	
